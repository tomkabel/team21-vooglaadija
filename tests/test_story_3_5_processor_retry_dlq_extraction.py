...
def test_outbox_relay_static_contracts_are_preserved() -> None:
    ...
    # The relay still filters on the pending status; require the actual query
    # predicate, not just the constant declaration.
    assert 'Outbox.status == "pending"' in source or "Outbox.status == _PENDING_STATUS" in source
