...
async def test_create_api_key_returns_token_and_prefix() -> None:
...
async def test_list_api_keys_excludes_secret() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await create_test_user_and_login(client)
        headers = await _auth_headers(client, token)
        await client.post(
...
async def test_api_key_authentication_works() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await create_test_user_and_login(client)
        headers = await _auth_headers(client, token)
...
async def test_revoke_api_key_invalidates_token() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await create_test_user_and_login(client)
        headers = await _auth_headers(client, token)
        created = await client.post(
...
async def test_revoke_unknown_key_returns_404() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await create_test_user_and_login(client)
...
async def test_read_only_key_cannot_write() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await create_test_user_and_login(client)
        headers = await _auth_headers(client, token)
        created = await client.post(
...
async def test_read_only_key_cannot_manage_keys() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await create_test_user_and_login(client)
        headers = await _auth_headers(client, token)
        created = await client.post(
...
async def test_wildcard_key_can_write() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await create_test_user_and_login(client)
        headers = await _auth_headers(client, token)
        created = await client.post(
...
async def test_unknown_pat_prefix_is_rejected() -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
